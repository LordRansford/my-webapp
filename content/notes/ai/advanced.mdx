...
Plain language. A way to score which parts of the input should influence the current step.  
Formal wording. A weighted sum of value vectors where weights come from query key similarity.  
...
Plain language. The slice of tokens the model can see at once.  
Formal wording. A fixed length sequence limit in the transformer architecture.  
...
Plain language. When probability estimates match reality.  
Formal wording. p(y=1|x)=0.7 should mean roughly 70% of such cases are true.  
...
Plain language. Fetch relevant documents, then ask the model to answer using them.  
Formal wording. Retrieve top k contexts, concatenate to prompt, decode tokens conditioned on combined context.  
...
Plain language. When an agent keeps acting in a harmful way because feedback is wrong or missing.  
Formal wording. Positive feedback without dampening leads to unbounded actions.  
...
<SectionHeader variant="content" emoji="🌫️" id="diffusion-generative">
  Diffusion models and generative systems
</SectionHeader>

<Callout variant="concept">
Diffusion is iterative denoising. The “magic” is a controlled reverse process guided by learned structure.
</Callout>

Diffusion-style generative systems are a different family from transformers, but the operational lesson is similar. High quality outputs can still be unsafe, biased, or misused. The system questions are not “is it impressive”, but “what can it generate”, “who can trigger it”, and “what evidence do we keep when it is abused”.

In a real system, diffusion models show up inside product features such as image generation, background removal, style transfer, and synthetic data for prototyping. The technical details matter less than the boundaries. Decide what inputs are allowed, what outputs are disallowed, and how you will detect attempts to route around restrictions. If you cannot describe the abuse cases, you cannot write the guardrails.

If you are deploying generation in a regulated or safety critical setting, treat it as a content pipeline. Provenance, review, and auditability. “We do not store prompts” sounds privacy friendly, but it can also erase the evidence you need to investigate misuse. A good standard is this. Store the minimum evidence that lets you explain and fix failures, and delete it on a clear schedule.
...
<SectionHeader variant="content" emoji="🧯" id="system-failure-modes">
...
A simple “gold standard” test is a rehearsal. Pick a plausible failure (bad retrieval result, tool misuse, prompt injection, or degraded performance), then walk through detection and containment. If the exercise ends with “we would notice via a customer complaint”, you have found a gap worth fixing before production.
...
Plain language. The risk that remains after controls.  
Formal wording. Risk not eliminated by mitigation, consciously accepted.  
