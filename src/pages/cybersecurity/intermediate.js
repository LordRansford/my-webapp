...
export default function Page({ source, headings, meta }) {
  ...
  const quizQuestions = useMemo(() => quizChapter3, []);
  ...
  const mdxComponents = useMemo(
    () => ({
      ...
      InputValidationSimulator,
      IncidentTimelineTool,
      QuizCard: () => <Quiz title="Intermediate knowledge check" questions={quizQuestions} />,
      PacketJourneyTool,
      LbtSimulator,
      InterceptionExplorer,
      DNSJourney,
  ...
  return (
    <NotesLayout
      pageKey="cybersecurity-chapter-2"
      title="Cybersecurity Notes"
      subtitle={meta?.description || "Chapter 2: Intermediate foundations."}
    >
      <nav className="panel stack" style={{ marginBottom: "1rem" }} aria-label="Cybersecurity pages">
        <div className="panel__header">
          ...
          <span className="muted">You are on Chapter 2 (Intermediate)</span>
        </div>
        <div className="control-row">
          {[
            ...
            <Link
              key={item.slug}
              href={item.slug}
              className={`pill ${item.slug === "/cybersecurity/intermediate" ? "pill--accent" : "pill--ghost"}`}
              aria-current={item.slug === "/cybersecurity/intermediate" ? "page" : undefined}
            >
      ...
      <div className="actions" style={{ marginTop: "1.5rem" }}>
        <Link href="/cybersecurity/advanced" className="button primary">
          Next: Chapter 3 (Advanced)
        </Link>
      </div>
    </NotesLayout>
  );
