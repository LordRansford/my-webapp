/**
 * Custom hook for managing habit planner state with localStorage persistence
 */

import { useState, useEffect, useCallback } from "react";
import { Habit } from "./HabitLibrary";
...
export interface HabitPlannerState {
...
const STORAGE_KEY = "ai-habit-planner";
 ...
 */
function loadState(): HabitPlannerState {
  ...
  try {
    ...
    if (stored) {
      ...
      const allCompletedDates = parsed.entries.flatMap((entry: HabitEntry) => entry.completedDates);
      const currentStreak = calculateStreak(allCompletedDates);
      const longestStreak = Math.max(
        currentStreak,
        parsed.longestStreak || 0,
        ...parsed.entries.map((entry: HabitEntry) => {
    ...
    console.error("Failed to load habit planner state:", error);
  }
 ...
 */
function saveState(state: HabitPlannerState): void {
  if (typeof window === "undefined") return;

  try {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
  } catch (error) {
    console.error("Failed to save habit planner state:", error);
  }
}

export function useHabitPlanner() {
  const [state, setState] = useState<HabitPlannerState>(loadState);
  ...
  }, [state.selectedHabit, state.customHabit, state.entries]);
  ...
  const getActiveEntry = useCallback((): HabitEntry | null => {
