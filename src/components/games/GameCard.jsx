export default function GameCard({ title, description, meta, children }) {
  return (
    <section className="group relative mb-6 overflow-hidden rounded-2xl border border-slate-200/80 bg-gradient-to-br from-white via-slate-50/40 to-white p-6 shadow-md transition-all duration-300 hover:border-slate-300/80 hover:shadow-lg">
      ...
      <div className="relative">
        {meta?.level || typeof meta?.minutes === "number" ? (
          <div className="mb-3 flex flex-wrap items-center gap-2 text-xs font-semibold text-slate-600">
            {meta?.level ? <span className="rounded-full bg-slate-100 px-2 py-0.5 text-slate-700">{meta.level}</span> : null}
            {typeof meta?.minutes === "number" ? (
              <span className="rounded-full bg-slate-100 px-2 py-0.5 text-slate-700">{meta.minutes} min</span>
            ) : null}
          </div>
        ) : null}
