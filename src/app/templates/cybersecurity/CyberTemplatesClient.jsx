...
export default function CyberTemplatesClient({ templates, areas }) {
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-12 md:px-6 lg:px-8">
      <section className="space-y-4 rounded-3xl border border-slate-200 bg-white/90 p-8 shadow-sm backdrop-blur">
        <p className="text-sm font-semibold uppercase tracking-wide text-slate-600">Templates - Cybersecurity</p>
        <h1 className="text-4xl font-semibold leading-tight text-slate-900">Cybersecurity templates</h1>
        <p className="max-w-3xl text-base text-slate-700">
          Built for security leads, architects, and responders who want structured, reusable workflows. Everything stays in the
          browser with exports ready when you need them.
        </p>
        <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-800">
          Each template follows the Template Standard: clear audience, instructions, editable workspace, outputs, and attribution
          notice. Autosave keeps your draft safe between visits. These resources are educational and planning aids, not legal
          advice, and not a substitute for professional security testing.
        </div>
      </section>

      <section className="mt-10 space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-2xl font-semibold text-slate-900">Cybersecurity tools</h2>
          <Link
            href="/support"
