...
export function coachingFromMetrics(m: Metrics): Coaching {
  const insights: string[] = [];

  if (m.overcorrectionIndex > 0.55) {
    insights.push("You change direction a lot in short bursts. That is overcorrection under pressure.");
  } else if (m.controlScore > 72) {
  ...
  if (m.errorClustering > 0.45) insights.push("Errors are clustering. After a mistake, slow your next two decisions.");

  const headline =
    m.overcorrectionIndex > 0.55
