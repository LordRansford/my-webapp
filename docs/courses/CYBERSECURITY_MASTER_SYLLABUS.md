...
| `cyber.foundations.f2-trust-boundaries-intro` | `foundations-f2-trust-boundaries-intro` | data flows + trust boundaries for a small system | Identify trust boundaries; propose proportionate mitigations | CSF: Identify; SSDF (design) |
| `cyber.foundations.f3-identity-basics` | `foundations-f3-identity-basics` | auth vs authz; MFA/passkeys basics (risk-based); least privilege; common shortcuts | Spot dangerous shortcuts; propose safer defaults | CSF: Protect; NIST 800-63 concepts |
| `cyber.foundations.f4-human-factors-phishing` | `foundations-f4-human-factors-phishing` | phishing mechanics; social engineering; incentives; insider basics | Identify phishing indicators; explain why “someone clicked” is incomplete | CSF: Protect/Detect |
| `cyber.foundations.f5-privacy-as-security` | `foundations-f5-privacy-as-security` | minimisation; retention; logging vs privacy trade-offs | Explain privacy as a security property; choose proportionate telemetry | CSF: Govern/Protect |
| `cyber.foundations.f6-transport-basics` | `foundations-f6-transport-basics` | HTTPS/TLS basics; cert chains; what TLS does/does not guarantee | Diagnose common TLS failure types at a high level | CSF: Protect |
| `cyber.foundations.f7-foundations-capstone` | `foundations-f7-foundations-capstone` | Foundations capstone + checkpoint | Produce a defensible evidence artefact | All above |
